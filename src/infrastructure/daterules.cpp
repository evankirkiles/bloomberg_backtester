...
std::tuple<int, int, int> TimeRules::get_time(BloombergLP::blpapi::Datetime date) {
    // Check the holidays first and use their corresponding opens and closes
    try {
        // If the market is closed on that day, increment the date by 1 day forwards
        if (time_holidays::holidays_US.at(date.year() - 1900).at(date.month() - 1).at(date.day()) == "CLOSED") {
            date.setDate();
        }
    }
}
